...
이거 로직을 바꿀꺼야. '수정하기'버튼을 누르면 현재 보여지는 허요오ip들이 input란에 들어가고 수정할 수 있어.
추가하기를 누르면 추가한게 input란으로 들어가고 
적용하기를 누르면 전체  ip가 api요청하고 요청된 ip로 div로 보여주는거야
...
const DrmAllowIpSection = ({ svcId }) => {
  ...
  const renderRow = (row) => {
    // const isEditing = editingId === row.ipId;
    return (
  ...
  const handleEditingMode = (modeStatus) =>{
    setIsEditing(modeStatus);
  }

  return (
    <div>
      <ContentHeader title="허용 IP 관리" $border={true} spacing={20} 
        extraContent={<Buttons.Outlined
            type={"grey"}
            onClick={() => handleEditingMode(!isEditing)}
            minWidth="80"
