...
import { Table as HdsTable } from 'signlw';
...
/**
 * @typedef {React.ComponentProps<typeof HdsTable> HdsTableProps} HdsTableProps
 */

/**
 * @typedef {Object} TableProps
 * @property {Array} [props.columns=[]] - 테이블 컬럼 정의 배열
 * @property {Array} [props.dataSource=[]] - 테이블 데이터 소스
 * @property {'blue'|'normal'|'borderless'} [props.type='blue'] - 테이블 스타일 타입
 * @property {Object} [props.pagination={}] - 페이징 설정
 ...
 * @property {React.ReactNode} [props.paginationRightExtraContent=null] - 페이지네이션 오른쪽 영역에 추가될 내용
 */
/**
 * @typedef {TableProps & Omit<HdsTableProps, keyof TableProps>} MergedProps
 */
 ...
 * @param {'blue'|'normal'|'borderless'} [props.type='blue'] - 테이블 스타일 타입
 * @param {Object} [props.pagination={}] - 페이징 설정
 ...
 * @param {React.ReactNode} [props.paginationRightExtraContent=null] - 페이지네이션 오른쪽 영역에 추가될 내용
 */
const Table = ({
  columns = [],
  dataSource = [],
  type = 'blue', // blue || normal || borderless
  pagination = {},
  paginationExtraContent,
  placeholderBorder = true,
  paginationRightExtraContent = null,
  ...props
}) => {
  ...
  useEffect(() => {
    ...
    const observer = new ResizeObserver(() => {
      const table = tableRef.current.querySelector('.signlw3-custom-table');
  ...
  return (
    <TableLayout ref={tableRef}>
      <StyledTable
        prefixCls="signlw3-custom-table"
        columns={mergedColumns}
        dataSource={dataSource}
        components={{ header: { cell: ResizableTitle } }}
        type={`${type !== undefined ? type : 'blue'}`}
        tableLayout={'fixed'}
        pagination={
          pagination === false
            ? false
        ...
        {...props}
      />
      {paginationRightExtraContent && dataSource.length > 0 ? (
        <div
          style={{
            position: 'absolute',
            right: 0,
...
const tableBlueType = css`
  .signlw3-custom-table-thead {
    .signlw3-custom-table-cell {
  ...
  .signlw3-custom-table-tbody {
    .signlw3-custom-table-row {
      .signlw3-custom-table-cell {
...
const tableBorderlessType = css`
  ...
  .signlw3-custom-table-tbody {
    ...
    .signlw3-custom-table-placeholder {
      .signlw3-custom-table-cell {
        border-bottom: ${(props) =>
          props.placeholderBorder === false ? 'none' : '1px solid #dddddd'};
      }
    }
  }
`;

const StyledTable = styled(HdsTable)`
  .${({ theme }) => theme.namespace} & {
    &.signlw3-custom-table-wrapper {
      .signlw3-custom-table {
        scrollbar-color: unset;

        .signlw3-custom-table-body {
          overflow-x: hidden;
          &::-webkit-scrollbar {
            width: 6px;
          }
        ...
        .signlw3-custom-table-thead {
          .signlw3-custom-table-cell {
            ...
            &.signlw3-custom-table-cell-scrollbar {
              box-shadow: none;
            }

            // filter css
            .signlw3-custom-table-filter-column {
            ...
            // sorters css
            .signlw3-custom-table-column-sorters {
              display: flex;
              align-items: center;
              ...
              .signlw3-custom-table-column-title {
                flex: 1;
                overflow: hidden;
              ...
              .signlw3-custom-table-column-sorter {
                margin-left: 0;
                margin-top: -4.4px;
                display: block;
                transform: unset;
